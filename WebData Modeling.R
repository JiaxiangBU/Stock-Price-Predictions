...
df$Class1 <-
  ifelse(
    df$Per_Change_T1 > .02,
    "Increase",
    ifelse(df$Per_Change_T1 < (-.02), "Decrease", "No_Change")
  )
df$Class5 <-
  ifelse(
    df$Per_Change_T5 > .02,
    "Increase",
    ifelse(df$Per_Change_T5 < (-.02), "Decrease", "No_Change")
  )
df$Class30 <-
  ifelse(
    df$Per_Change_T30 > .01,
    "Increase",
    ifelse(df$Per_Change_T30 < (-.02), "Decrease", "No_Change")
  )
# df$Class
...
mod <- lm(
  Per_Change_T1 ~ Finance.Headline + Finance.First.Paragraph +
...
summary(mod)
sqrt(mean((
  te$Per_Change_T1 - predict(mod, newdata = te)
) ^ 2))

modgent1 <- lm(
  Per_Change_T1 ~ General.Headline + General.First.Paragraph +
...
summary(modgent1)
sqrt(mean((
  te$Per_Change_T1 - predict(modgent1, newdata = te)
) ^ 2))
...
mod <-
  lm(
    Per_Change_T5 ~ Industry + Finance.Headline + Finance.First.Paragraph +
      Finance.Last.Paragraph + mkvaltq + Delta
    ,
    data = tr
  )
...
summary(mod)
sqrt(mean((
  te$Per_Change_T5 - predict(mod, newdata = te)
) ^ 2))

modgen <- lm(
  Per_Change_T5 ~ General.Headline + General.First.Paragraph +
    General.Last.Paragraph + mkvaltq + Delta
  ,
  data = tr
)
summary(modgen)
sqrt(mean((
  te$Per_Change_T5 - predict(modgen, newdata = te)
) ^ 2))
...
mod <-
  lm(
    Per_Change_T30 ~ Industry + Finance.Headline + Finance.First.Paragraph +
      Finance.Last.Paragraph + mkvaltq + Delta
    ,
    data = tr
  )
summary(mod)
sqrt(mean((
  te$Per_Change_T30 - predict(mod, newdata = te)
) ^ 2))


modgen <- lm(
  Per_Change_T30 ~ General.Headline + General.First.Paragraph +
    General.Last.Paragraph + mkvaltq + Delta
  ,
  data = tr
)
summary(modgen)
sqrt(mean((
  te$Per_Change_T30 - predict(modgen, newdata = te)
) ^ 2))
...
#######################################################
mod <-
  lm(Per_Change_T1 ~ Industry + Finance.Combined + mkvaltq + Delta,
     data = tr)
summary(mod)
sqrt(mean((
  te$Per_Change_T1 - predict(mod, newdata = te)
...
summary(modgen)
sqrt(mean((
  te$Per_Change_T1 - predict(modgen, newdata = te)
) ^ 2))
...
#################
mod <-
  lm(Per_Change_T5 ~ Industry + Finance.Combined + mkvaltq + Delta,
     data = tr)
summary(mod)
sqrt(mean((
  te$Per_Change_T5 - predict(mod, newdata = te)
...
#########
mod <-
  lm(Per_Change_T30 ~ Industry + Finance.Combined + mkvaltq + Delta,
     data = tr)
summary(mod)
sqrt(mean((
  te$Per_Change_T30 - predict(mod, newdata = te)
